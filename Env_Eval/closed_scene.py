...
from Env_Config.Utils_Project.utils import (
    get_unique_filename,
    closed_scene_judge_final_all_poses,
)
...
def assertx (cond, msg="assertion failed"):
    if not cond:
        ...
        sys.stderr.write(f"[ASSERTX FAILED] {msg} at {filename}:{lineno}\n")
        sys.stderr.flush()
        os._exit (1)
...
class ClosedSceneEnv (BaseEnv):
    def __init__ (self):
        ...
        self.garment_index, _, _ = closed_scene_judge_final_all_poses (
        ...
        self.top_camera.create_mp4 ("Results/closed_scene_tmp/initial.mp4")
    ...
    def model_pick_one_step (self):
        ...
        self.top_camera.get_rgb_graph (
            save_or_not = True,
            save_path = "./Results/closed_scene_tmp/current_scene.jpg",
        )
        ...
        _, _, idx_choosed, masks = solver (
            camera = self.top_camera,
            img_path = "./Results/closed_scene_tmp/current_scene.jpg",
            prompt_type = "closed_scene_all_garments",
            use_depth_info = True,
            use_sam2 = False,
            exist_masks = True,
            labels = labels,
        ...
        # pick_point[-1] += 0.35
        # if master == 0:
        #     assertx (self.franka.left.move_to (pick_point), "franka wrong")
        # else:
        #     assertx (self.franka.right.move_to (pick_point), "franka wrong")
        # print ("ok master reach up\n")
        ...
        # else:
        #     assertx (self.franka.right.move_to (pick_point), "franka wrong")
        # print ("ok master reach down\n")
        
        if master == 0:
            self.set_attach_to_garment (attach_position = [pick_point, None], indices = [0])
        #     self.franka.left.close ()
        else:
            self.set_attach_to_garment (attach_position = [None, pick_point], indices = [1])
        #     self.franka.right.close ()

        pick_point[-1] += 0.75
        if master == 0:
            # assertx (self.franka.move_both_with_blocks (pick_point, None, None, None, attach = self.attach, indices = [0]), "franka wrong")
            # assertx (self.franka.move_both_with_blocks (np.array (self.config.target_positions[0]), None, None, None, attach = self.attach, indices = [0]), "franka wrong")
            ...
            # assertx (self.franka.move_both_with_blocks (None, None, pick_point, None, attach = self.attach, indices = [1]), "franka wrong")
            # assertx (self.franka.move_both_with_blocks (None, None, np.array (self.config.target_positions[1]), None, attach = self.attach, indices = [1]), "franka wrong")
        ...
        for i in range (150):
            self.world.step ()
        self.top_camera.create_mp4 (f"Results/closed_scene_tmp/pick.mp4")
        self.recording_camera.capture = False
        ...
        self.top_camera.get_rgb_graph (
            save_or_not = True,
            save_path = f"Results/closed_scene_tmp/picked_scene.jpg"
        )

        self.garments.set_invisible_to_secondary_rays (False)
        labels, masks = processor_sam2video (
            camera = self.top_camera,
            video_path = 'Results/closed_scene_tmp/pick.mp4',
            use_depth_info = True,
        ...
        multiple_picked_garments = solver (
            camera = self.top_camera,
            img_path = "Results/closed_scene_tmp/picked_scene.jpg",
        ...
        if multiple_picked_garments:
            ...
            if master == 0:
                ...
                # self.franka.left.open ()
            else:
                self.attach.detach (1)
                # self.franka.right.open ()
                
            # if master == 0:
            #     self.franka.left.move_to (ori_left_franka[0], ori_left_franka[1])
            # else:
            #     self.franka.right.move_to (ori_right_franka[0], ori_right_franka[1])
        ...
        if need_right_arm == True:
            if master == 0:
                ...
                # assertx (self.franka.right.move_to (pick_point_right), "franka wrong")
            else:
                self.set_attach_to_garment (attach_position = [pick_point_right, None], indices = [0])
                # assertx (self.franka.left.move_to (pick_point_right), "franka wrong")
            ...
            tar_position[-1] += 0.85
            # assertx (self.franka.move_both_with_blocks (tar_position - np.array ([0.16, 0.005, 0]), self.tar_ori_left,
            #                                     tar_position - np.array ([ - 0.16, 0.005, 0]), self.tar_ori_right,
            #                                     attach = self.attach, indices = [0, 1]), "franka wrong")
            ...
            print ("ok franka up\n")
            tar_position[-1] -= 0.3
            # assertx (self.franka.move_both_with_blocks (tar_position - np.array ([0.16, 0.005, 0]), None,
            #                                     tar_position - np.array ([ - 0.16, 0.0505, 0]), None,
            #                                     attach = self.attach, indices = [0, 1]), "franka wrong")
            self.move_block ([tar_position - np.array ([0.16, 0.01, 0]),
                              tar_position - np.array ([ -0.16, 0.01, 0])],
                                attach = self.attach, indices = [0, 1])
            ...
            tar_position[-1] += 0.85
            if master == 0:
                # assertx (self.franka.move_both_with_blocks (tar_position - np.array ([0.16, 0.005, 0]), self.tar_ori_left, None, None, attach = self.attach, indices = [0]), "franka wrong")
                self.move_block ([tar_position - np.array ([0.16, 0.01, 0])], attach = self.attach, indices = [0])
            else:
                # assertx (self.franka.move_both_with_blocks (None, None, tar_position - np.array ([ - 0.16, 0.005, 0]), self.tar_ori_right, attach = self.attach, indices = [1]), "franka wrong")
                self.move_block ([tar_position - np.array ([ -0.16, 0.01, 0])], attach = self.attach, indices = [1])
            print ("ok franka up\n")
            
            tar_position[-1] -= 0.3
            if master == 0:
                # assertx (self.franka.move_both_with_blocks (tar_position - np.array ([0.16, 0.005, 0]), None, None, None, attach = self.attach, indices = [0]), "franka wrong")
                self.move_block ([tar_position - np.array ([0.16, 0.01, 0])], attach = self.attach, indices = [0])
            else:
                # assertx (self.franka.move_both_with_blocks (None, None, tar_position - np.array ([ - 0.16, 0.005, 0]), None, attach = self.attach, indices = [1]), "franka wrong")
                self.move_block ([tar_position - np.array ([ -0.16, 0.01, 0])], attach = self.attach, indices = [1])
            print ("ok franka down\n")
        ...
        # if need_right_arm == True:
        #     self.franka.dense_move_both_ik (ori_left_franka[0], ori_left_franka[1],
        #                                     ori_right_franka[0], ori_right_franka[1])
        # else:
        #     if master == 0:
        #         self.franka.left.move_to (ori_left_franka[0], ori_left_franka[1])
        #     else:
    ...
    def adjust_mask (self,
        ...
        # ori_left_franka = self.franka.left.get_cur_ee_pos ()
        ...
        for i, pick_point in enumerate (pick_points):
            ...
            # pick_point[-1] += 0.35
            # assertx (self.franka.left.move_to (pick_point), "franka wrong")
            # print ("ok franka reach up [adjust]\n")
            # pick_point[-1] -= 0.35
            # assertx (self.franka.left.move_to (pick_point), "franka wrong")
            # print ("ok franka reach down [adjust]\n")
            # self.franka.left.close ()
        ...
        self.top_camera.create_mp4 ("Results/closed_scene_tmp/stir.mp4")

        # sam2 => regen mask
        self.garments.set_invisible_to_secondary_rays (False)
        labels, masks = processor_sam2video (
            camera = self.top_camera,
            video_path = 'Results/closed_scene_tmp/stir.mp4',
...
if __name__ == "__main__":
    Path ("Results/closed_scene_tmp").mkdir (parents = True, exist_ok = True)

    video_path = "Results/whole_procedure/closed_scene"
