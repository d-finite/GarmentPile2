...
class DefaultConfig(object):
    ...
    max_epoch = 300  # 训练轮数_300
    lr = 0.0009  # 初始化学习率
    lr_decay = 0.99  # 学习率衰减，lr = lr * lr_decay，随着训练的进行逐渐减小学习率的大小，使得模型在训练后期更容易收敛到全局最优解而不是在最优解附近振荡
    weight_decay = 2e-4  # 权重衰减，一种正则化技术，通过向损失函数添加一个惩罚项来减小模型的权重值，以防止过拟合，提高模型的泛化能力
    betas = (0.9, 0.999)  # 两个超参数，分别控制一阶和二阶矩估计的指数衰减率。
    eps = 1e-08  # 用于数值稳定性的小常数。它在计算更新时用于防止除以零的错误。
    momentum = 0.85  # 动量因子。动量可以加速 SGD 在相关方向上的收敛，并减少震荡。通常设置在 0 和 1 之间。
