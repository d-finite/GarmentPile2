...
from Model.pn2_aff import Aff_Model, Aff_Model_Loss
from Data_Utils.dataloader import DataLoader_AffModel
...
def train(**kwargs):
    ...
    wandb.init(
        project="DualPile-PN2",
        name="Closed_Scene_Aff_Model",
        config=config,
        # mode="online",
    )
    ...
    train_data = DataLoader_AffModel(mode="train", data_dir="Model_Train/Data/closed_scene/train")
    val_data = DataLoader_AffModel(mode="val", data_dir="Model_Train/Data/closed_scene/val")
    ...
    for epoch in range(opt.max_epoch):
        ...
        for step, (data, label) in enumerate(processbar):
            ...
            label = label.to(opt.device).view(-1, 1)
            # reset grad
            model.zero_grad()
            # print(data.shape)
        ...
        if (epoch + 1) % opt.print_freq == 0:
            if not os.path.exists("Model_Train/Model_Checkpoints/closed_scene"):
                os.makedirs("Model_Train/Model_Checkpoints/closed_scene")
            torch.save(
                model.state_dict(),
                f"Model_Train/Model_Checkpoints/closed_scene/closed_scene_{epoch+1}.pth",
            )
