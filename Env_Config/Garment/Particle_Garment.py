...
class Particle_Garment:
    def __init__(
        ...
        visual_material_usd:str=None, #"Assets/Material/Garment/linen_Pumpkin.usd",
        ...
        contact_offset:float=0.008,             # important parameter
        rest_offset:float=0.0075,                # important parameter
        particle_contact_offset:float=0.008,    # important parameter
        fluid_rest_offset:float=0.0075,
        solid_rest_offset:float=0.0075,
        adhesion:float=0.0,                     # important parameter
        adhesion_offset_scale:float=0.0,        # important parameter
        cohesion:float=1.0,                     # important parameter
        particle_adhesion_scale:float=0.1,      # important parameter
        particle_friction_scale:float=0.1,      # important parameter
        drag:float=0.0, 
        lift:float=0.1, 
        friction:float=0.5,                    # important parameter
        damping:float=0.0,   
        # gravity_scale:float=1.0,                # important parameter
        particle_mass:float=0.1,               # important parameter
        self_collision:bool=True, 
        self_collision_filter:bool=True, 
        stretch_stiffness:float=1e15, #1e6  
        ...
        self.position = pos
        self.orientation = euler_angles_to_quat(ori, degrees=True)
        self.scale = scale
        ...
        self.particle_system_1 = self.particle_system
        self.particle_material_1 = self.particle_material
        ...
        physicsUtils.add_physics_material_to_prim(
            self.stage, 
            self.stage.GetPrimAtPath("/World/Garment/particleSystem"), 
            "/World/Garment/particleMaterial"
        )
        self.particle_system_0 = self.particle_system
        self.particle_material_0 = self.particle_material
        ...
        _, self.points_num = self.get_cur_pose()
        
        prim = self.stage.GetPrimAtPath(self.garment_mesh_prim_path)
        self.mesh = UsdGeom.Mesh(prim)
        if not UsdGeom.PrimvarsAPI(self.mesh).GetPrimvar("doNotCastShadows"):
            UsdGeom.PrimvarsAPI(self.mesh).CreatePrimvar("doNotCastShadows", Sdf.ValueTypeNames.Bool)    # Hide from secondary rays
        if not UsdGeom.PrimvarsAPI(self.mesh).GetPrimvar("invisibleToSecondaryRays"):
            UsdGeom.PrimvarsAPI(self.mesh).CreatePrimvar("invisibleToSecondaryRays", Sdf.ValueTypeNames.Bool)    # Hide from secondary rays
            
        self.set_secondary_rays(True)
            
    def set_secondary_rays(self, flag:bool=True):
        try:
            UsdGeom.PrimvarsAPI(self.mesh).GetPrimvar("doNotCastShadows").Set(flag)
            UsdGeom.PrimvarsAPI(self.mesh).GetPrimvar("invisibleToSecondaryRays").Set(flag)
        except Exception:
            # print ("this garment is already deleted")
            pass

    def set_particle_system(self, index=0):
        if index == 0:
            self.particle_system = self.particle_system_0
            self.particle_material = self.particle_material_0
        elif index == 1:
            self.particle_system = self.particle_system_1
            self.particle_material = self.particle_material_1
    ...
    def get_cur_pose(self):
        from ..Utils_Project.utils import get_all_mesh_points
        # return np.mean(get_all_mesh_points(self.stage, self.garment_prim_path), axis=0)
        points = get_all_mesh_points(self.stage, self.garment_prim_path)
        return np.mean(points, axis=0), points.shape[0]
...
def find_unique_string_name(initial_name: str, is_unique_fn) -> str:
    iterator = 0
