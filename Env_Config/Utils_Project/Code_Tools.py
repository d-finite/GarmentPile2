...
def dense_trajectory_points_generation (
    ...
    tck, u = splprep (initial_sample_points.T, s = 0)
    u_new = np.linspace (0, 1, num_points)
    interp_pos = np.array (splev (u_new, tck)).T 
    ...
    if start_quat is not None and end_quat is not None:
        ...
        interp_ori = slerp (interp_times).as_quat ()
        return interp_pos, interp_ori
